...
from AmbroMusic.utils.decorators import (ActualAdminCB, AdminActual,
                                         language)
from AmbroMusic.utils.formatters import alpha_to_int
...
@app.on_message(
    filters.command(["RELOAD_COMMAND"]) | filters.command(["ترسيت","ريلود"],prefixes= ["/",""])
    & filters.group
    & ~filters.edited
    & ~BANNED_USERS
)
...
@app.on_message(
    filters.command(["RESTART_COMMAND"]) | filters.command(["ريستارت","ريست"],prefixes= ["/",""])
    & filters.group
    & ~filters.edited
    & ~BANNED_USERS
)
